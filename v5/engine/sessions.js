...
const SESSIONS = (function(){
  ...
  function analyzeSession(sessionId,context,natalD1,dasha,transitReport){
    ...
    const dashaModule=typeof require==='function'?require('./dasha'):DASHA;
    const dashaI=dasha?dashaModule.interpret(dasha.maha,dasha.antar):{sc:0};
    const panSc=pan.score||0;

    // Session direction follows the natal kundli/dasha and transits. Hora is display context only.
    const natalStrength=(natalD1?.planets||[]).reduce((s,p)=>s+(p.nat==='benefic'?p.strength-50:50-p.strength),0)/100;
