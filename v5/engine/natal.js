/* ============================================================
   NASDAQ Vedic System V5 — engine/natal.js
...
const NATAL = (function(){
  ...
  function mercuryLon(jd){
    ...
    const Mr=M*D2R;
    return n360(L+23.44*Math.sin(Mr)+2.9818*Math.sin(2*Mr)+0.5255*Math.sin(3*Mr)+0.1058*Math.sin(4*Mr));
  }
  ...
  function getPlanetLon(eng,jd){
    switch(eng){
      case'Sun':return sunLon(jd);case'Moon':return moonLon(jd);
      case'Mercury':return mercuryLon(jd);case'Venus':return venusLon(jd);
      case'Mars':return marsLon(jd);case'Jupiter':return jupiterLon(jd);
      case'Saturn':return saturnLon(jd);case'Rahu':return rahuLon(jd);
      case'Ketu':return n360(rahuLon(jd)+180);
      case'Uranus':return uranusLon(jd);case'Neptune':return neptuneLon(jd);
      default:return 0;
    }
  }
  ...
  function getDignity(eng,rashi){
    ...
    const OW={Sun:[4],Moon:[3],Mercury:[2,5],Venus:[1,6],Mars:[0,7],Jupiter:[8,11],Saturn:[9,10]};
  ...
  function calcLagna(jd,lat,lon){
    ...
    let asc=Math.atan2(Math.cos(LST*D2R),-(Math.sin(LST*D2R)*Math.cos(eps*D2R)+Math.tan(lat*D2R)*Math.sin(eps*D2R)));
    return n360(asc*R2D);
  }
  ...
  function buildD1(birth){
    ...
    const planets=PLANETS_DEF.map(pd=>{
      ...
      return{...pd,tropLon,sidLon,rashi,deg,nak,nakPada,retro,dignity,strength,house,jd};
    });
    ...
    return{jd,ayan,lagnaSid,lagnaSign,planets,houses,birth,schemaVersion:"5.0"};
  }

  const BHAVA_MKT=[2,3,1,1,5,-3,2,-5,4,0,5,-4];
  const BHAVA_SIG=['बाज़ार खुलना','धन/आय','संचार/टेक','आधार/समर्थन',
  ...
  function analyze12Houses(d1,dasha,transits){
    const activations=d1.houses.map((h,i)=>{
      ...
      return{house:i+1,sign:h.sign,lord:h.lord,signif:BHAVA_SIG[i],
        mktScore:mkt,placementScore:parseFloat(placementScore.toFixed(2)),
        lordStrength:lordPlanet?.strength||0,lordScore:parseFloat(lordScore.toFixed(2)),
    ...
    const topBullish=activations.filter(a=>a.mktScore>0).sort((a,b)=>b.activationScore-a.activationScore)[0];
    const topBearish=activations.filter(a=>a.mktScore<0).sort((a,b)=>a.activationScore-b.activationScore)[0];

    return{activations,topBullish,topBearish,
      overallHouseScore:activations.reduce((s,a)=>s+a.activationScore,0)};
  }

  // NASDAQ natal chart (precomputed reference)
  const NASDAQ_BIRTH={year:1971,month:2,day:8,hour:10,minute:0,lat:40.714,lon:-74.006,tz:-5};

  function getNasdaq(){
    const birth=typeof CONFIG!=='undefined'&&CONFIG.NASDAQ_BIRTH?CONFIG.NASDAQ_BIRTH:NASDAQ_BIRTH;
    return buildD1(birth);
  }

  return{JD,n360,n180,lahiri,getPlanetLon,isRetro,getDignity,
    getPlanetStrength,calcLagna,buildD1,analyze12Houses,
    getNasdaq,NASDAQ_BIRTH,PLANETS_DEF,RASHI_LORDS,BHAVA_MKT,BHAVA_SIG};
})();
